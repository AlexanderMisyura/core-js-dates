...
function getTime(date) {
  ...
  const hours = formatTimeUnit(dateObj.getHours());
  const minutes = formatTimeUnit(dateObj.getMinutes());
  const seconds = formatTimeUnit(dateObj.getSeconds());
...
function formatDate(date) {
  const dateObj = new Date(date);
  return dateObj.toLocaleString('en-US', { timeZone: 'UTC' });
}
...
function getWeekNumberByDate(date) {
  ...
  const firstDay = startDateObj.getDay() === 0 ? 7 : startDateObj.getDay();
  let week = firstDay <= 4 ? 1 : 0;
